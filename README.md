...
npm run test:e2e   # builds, then runs Playwright against the export
npm run sync:author # refresh the footer's name and avatar from GitHub
```

`test:e2e` builds first on purpose. The browser tests run against the
static export in `out/`, served by `scripts/serve-out.mjs` on port **3100**
— the artifact that actually deploys, rather than `next dev`, which is a
different program producing a different bundle. It is also faster: a full
run of both viewports takes about 31s instead of 52s. Port 3100 rather than
3000 so that a dev server left open cannot be silently reused as the
subject under test.

Playwright runs two projects: `Desktop Chrome` for `e2e/smoke.spec.ts`, and
`Pixel 7` for `e2e/mobile.spec.ts`, which covers only what differs on a
phone — overflow, tap-target size, modal height. Whether a perk rolls is
the same on both and is tested once.
...
capture:screenshots` (needs `npm run dev` running on port 3000 — the
  screenshot script drives the dev server, unlike the e2e suite, which
  drives the built export on 3100) regenerates all of them automatically
  via Playwright.
- `data/author.json` and `public/author-avatar.webp` are generated by `npm
run sync:author`. The footer reads them directly instead of calling the
  GitHub API on every page load, which was rate-limited to 60 requests an
  hour per IP and answered 403 far more often than it answered anything
  useful.
