...
подтягивается напрямую с официальной wiki скрапером, а не хардкодится
руками. Раньше жил как раздел [Vortex
...
- 📊 Локальная статистика роллов, PWA (устанавливается, работает офлайн)
...
Данные из скрапера — не всегда финальная истина: когда собственная
страница Fandom-вики у конкретного предмета/аддона временно не в
порядке (например, у Fandom сломался шаблон описания сразу после
выхода патча) или иконка ещё не загружена, `scripts/scrape-loadout.ts`
умеет точечно подставить исправление из другого источника (например,
[deadbydaylight.wiki.gg](https://deadbydaylight.wiki.gg)) через
небольшую карту известных исключений в самом скрипте — вместо того,
чтобы молча показывать битую картинку или служебный текст ошибки шаблона.
...
или сбросить выбор. Список персонажей выводится из тех же данных, что и
сами перки (`Perk.character`), без отдельного хардкод-списка.
...
Скачанные иконки кэшируются по фактическому URL-источнику на вики
(`data/icon-sources.json` / `data/loadout-icon-sources.json`) — не
просто «файл уже есть», а именно «источник не изменился» — так
редизайн иконки на вики подхватится на следующем прогоне, а не
останется незамеченным навсегда.
