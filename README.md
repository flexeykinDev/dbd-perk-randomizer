...
предмета — вместо него в интерфейсе показывается иконка их Силы). Каждый
слот (Предмет/Аддоны/Подношение) включается и отключается независимо.
...
редиректы самой вики (не по жёстко зашитому списку — так новый убийца
подхватывается автоматически, без правок скрипта), и отдельно заходит на
страницу каждого убийцы за иконкой его Силы (раздел «Power:» на
странице персонажа).

Предметы из категории «только карта/событие» (Eye of Vecna, Lament
Configuration, Keycard и т.д. — на вики помечены «Limited Item») в пул не
попадают: игрок их не выбирает сам, они появляются в трайле по игровому
сценарию конкретной главы, так что рандомайзер не может честно
предложить их как выбор экипировки.

### Локализация экипировки (предметы, аддоны, подношения)

Так же, как и у перков, русские названия и описания подтягиваются с
[русской вики](https://dead-by-daylight.fandom.com/ru/), а не переводятся
руками — но добавить пришлось не один скрипт, а три, потому что вики
раскладывает эти три категории по-разному:
...
- **Предметы и подношения** — у каждого своя страница на вики, как у
  перков: `sync-loadout-localization.ts` ищет русское название через
  `action=opensearch` и проверяет совпадение по категории
  («Категория:Предметы» / «Категория:Подношения…»), `sync-loadout-descriptions.ts`
  затем берёт описание с той же страницы. У предметов и подношений разная
  разметка страницы (раздел «Особенности» у предметов, «Использование» у
  подношений, а у части предметов вообще нет отдельного раздела —
  описание сразу после инфобокса) — скрипт пробует оба варианта, а не
  считает, что разметка одна на всех.
- **Аддоны** — отдельной страницы на каждый аддон не существует: вики
  группирует их по предмету/силе убийцы на одну страницу (например,
  «Медвежий капкан (улучшения)» — все аддоны Ловца сразу), так же, как
  собрана и английская страница Add-ons. Поэтому `sync-loadout-addons.ts`
  не ищет каждый аддон отдельно, а читает все ~50 групповых страниц
  (список — из «Категория:Улучшения», без ручного списка убийц) и
  сопоставляет каждую строку таблицы с нужным аддоном по английскому
  названию, которое сама вики пишет рядом с русским («Кровавая
  пружина(англ. Bloody Coil)») — а не по порядку строк или другой догадке,
  которая могла бы молча перепутать аддон одного убийцы с другим.

Сопоставление аддонов сейчас покрывает ~91% (у части, в основном самых
новых убийц, вики ещё не завела групповую страницу) — недостающие
показывают английский текст с честной пометкой в карточке, а не
обрываются с ошибкой.

### Случайный персонаж

Кнопка **Случайный персонаж** (рядом с фильтром темы/слотами экипировки)
роллит персонажа своей роли и показывает его портрет рядом с билдом —
крестик рядом убирает выбор. Список персонажей выводится из тех же
данных, что и сами перки (`Perk.character`), без отдельного
хардкод-списка.
...
- **В режиме Экипировки, для убийцы:** сам выбор персонажа — это и есть
  результат: кнопка решает, чьи аддоны Силы будут роллиться, вместо
  внутреннего случайного выбора. Портрет и Сила в HUD всегда совпадают.
  Для выжившего предметы/аддоны не привязаны к конкретному персонажу
  (как и в самой игре), так что там это чисто визуальный выбор.

### Оверлей для OBS

Кнопка **Оверлей OBS** открывает модалку с прозрачной ссылкой вида
`.../#/obs` — добавь её как источник **Browser** в OBS (800×220), и она
будет в реальном времени показывать те же карточки перков, что сейчас на
...
|---|---|---|---|
| `scale` | `50`–`200` | `100` | Размер карточек, % (крутится слайдером в модалке) |
...
не нужно. Ссылки, сохранённые ещё до появления слайдера (со старым
параметром `size=sm/md/lg`), тоже продолжают работать — просто пересчитываются
в эквивалентный `scale`.
...
свой кулдаун вместо фиксированных 4 секунд).
...
- `data/perks.json`, `data/meta.json`, `data/characters.json`,
  ...
  следующем прогоне. То же касается `data/items.json`, `data/addons.json`,
  `data/offerings.json`, `data/loadout-meta.json`, `data/loadout-ids.json`
  и `data/killer-power-icons.json` (`npm run scrape:loadout`).
