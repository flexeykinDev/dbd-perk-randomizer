...
/* Reading the wiki's prose: splitting a lore quote off the end, dropping the
 * boilerplate, breaking the rest into sentences, deciding which of those state
 * a mechanic, and marking up the numbers.
 *
 * All of it is pattern-matching against text written by wiki editors, which is
 * why nearly every regex here carries a note about the entry that forced it.
 * Nothing in this file knows what a perk is. */
...
export function splitQuote(text: string, lang: Lang): { body: string; quote: string | null } {
...
export const SECRET_OFFERING_RE =
...
export function isMechanical(sentence: string): boolean {
  return (
    /\d|%/.test(sentence) ||
...
export function isGlossaryAside(sentence: string): boolean {
...
export function stripLoreIntro(sentences: string[]): string[] {
...
export const SECRET_BULLET = "**Secret**.";
