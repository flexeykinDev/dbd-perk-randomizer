...
- [Закрепление и переброс одного слота](#закрепление-и-переброс-одного-слота)
- [Готовые билды](#готовые-билды)
- [Задание дня](#задание-дня)
...
- 📌 **Закрепление** перка, чтобы он пережил следующий ролл, и
  переброс одного слота, не трогая остальные три
- 📖 **Готовые билды** — восемь собранных архетипов (гонка генераторов,
  порчи, давление в погоне…) как отправная точка
...
## Закрепление и переброс одного слота

На каждой карточке перка есть замочек и кубик, оба доступны и с
клавиатуры.

| Клавиша         | Действие                     |
| --------------- | ---------------------------- |
| `Space`/`Enter` | Сгенерировать билд целиком   |
| `1`–`4`         | Перебросить только этот слот |
| `C`             | Скопировать билд             |
| `S`             | Скопировать ссылку           |

**Закрепление** держит перк в его слоте: `Space` перебрасывает всё
вокруг него — то, что нужно, когда три перка из четырёх уже подходят.
**Переброс слота** — зеркальная операция: меняет ровно один перк и не
трогает остальные. Закреплённый слот не перебрасывается, а снятие
замочка оставляет билд как есть, а не открывает под ним другой перк;
полный ролл сбрасывает перебросы слотов, но сохраняет закрепления.

Закрепления живут только в текущей сессии и намеренно не попадают в
ссылку — ссылка и так описывает готовый билд, и получатель в любом
случае увидит те же четыре перка.

Горячие клавиши не мешают, пока вы печатаете в поле или открыто
модальное окно, и никогда не перебивают браузерные сочетания (всё с
`Ctrl`/`Cmd`/`Alt` проходит насквозь). В билдах по сиду и по ссылке
оба контрола скрыты — такие билды фиксированы по определению.

## Готовые билды

**Ещё → Готовые билды** — по четыре собранных набора на роль: те
архетипы, которые игроки и так знают по именам (гонка генераторов,
второй шанс, порчи, давление в погоне и так далее). Они закрывают
случай, с которым рандомайзер плохо справляется: перков 300+, и совсем
непонятно, какие из них дружат между собой.

Применение идёт по тому же пути «общего билда», что ссылка-шеринг и
`!paste` из Twitch, — то есть готовый билд ведёт себя ровно как
присланный кем-то: показывается как есть, шерится ссылкой и заменяется,
как только вы нажмёте «Сгенерировать». Это отправная точка, а не режим;
если нужен _тематический случайный_ билд — используйте фильтр по тегам
в пуле.

Наборы лежат в [`data/build-presets.json`](data/build-presets.json) и
написаны руками, а не собраны классификатором тегов: тот работает по
ключевым словам, и билд из него читается как «четыре перка, где
упоминается генератор», а не как билд, который кто-то стал бы играть.

Значит, это ручной контент, а он обычно протухает: данные перков
обновляет скрапер по расписанию, и переименованный перк оставил бы
набор молча с тремя перками вместо четырёх. `lib/build-presets.test.ts`
проверяет каждый слаг по актуальным данным на каждом `npm test` — так
что устаревший набор роняет CI, а не тихо ломается.

## Задание дня

Все, кто открыл задание дня в один и тот же день по UTC, получают
одинаковый билд, а в строке активного сида видно, сколько игроков его уже
прошли:

> Активный сид: `2026-08-19-survivor` · сегодня сыграли 42 игрока

Считается именно **участие, а не заходы**. Инкремент при загрузке страницы
был бы проще — и означал бы, что число говорит «людей открыли сайт», пока
подпись говорит «сыграли сегодня»; плюс запись в общую базу шла бы за
каждого случайного посетителя, который до функции даже не дотронулся.
Счётчик растёт, когда человек действительно переключает доску в режим
задания дня, один раз на браузер в сутки.

Анонимно по устройству: хранится только целое число за день по пути
`daily-challenge/<YYYY-MM-DD>/count`. Никаких записей о посетителях — ни в
базе, ни в браузере, кроме флага «я сегодня уже посчитан».

Всё это тихо деградирует: блокировщик, офлайн или правила базы, не дающие
доступ к этому пути, ничего не стоят игроку — счётчик просто не появится.
Ноль не показывается никогда: только начавшийся день по UTC и недоступная
база выглядят из браузера одинаково, и ни то ни другое не повод сообщать,
что сайт мёртв. У своего сида счётчика нет вовсе — этот билд только ваш.
...
- **В режиме Экипировки, для убийцы:** сам выбор персонажа — это и есть
  ...
  а не показывает все ~900 аддонов сразу. Для выжившего предметы/аддоны
